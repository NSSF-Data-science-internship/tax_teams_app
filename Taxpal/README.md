...
## Hybrid vector RAG and GraphRAG

TaxPal keeps Chroma/BGE-M3 vector retrieval as its default and optionally augments it with Microsoft GraphRAG. Direct questions such as rates and definitions stay on the faster vector path. Relationship, amendment, comparison, and cross-corpus questions are routed to local or global graph search. If the graph service or index is unavailable, TaxPal records the diagnostic and continues with vector and trusted-web evidence.

The GraphRAG service runs on port `8002`. Its index is generated under `graphrag/output` and is intentionally excluded from Git. Before indexing, ensure `GEMINI_API_KEY` is set in `env/.env.local`. Graph indexing makes model calls and may consume substantial quota, so start with a small pilot:

```powershell
docker compose build graph-search
docker compose run --rm graph-search python graph_index.py `
  --root /graph `
  --chunks /data/chunks/all_chunks.json `
  --max-chunks 25 `
  --index
docker compose up -d graph-search
```

Check readiness at `http://localhost:8002/health`. The response changes from `"indexed": false` to `"indexed": true` after all required Parquet tables have been produced. Once the pilot is working, build the complete corpus by repeating the indexing command without `--max-chunks 25`.

Relevant configuration variables:

- `GRAPH_RAG_ENABLED=true` enables hybrid routing.
- `GRAPH_RAG_MODE=auto` uses graph retrieval only when the question benefits from it; `local`, `global`, and `drift` force a method.
- `GRAPHRAG_COMPLETION_MODEL` defaults to `gemini-2.5-flash-lite`.
- `GRAPHRAG_EMBEDDING_MODEL` defaults to `gemini-embedding-001`.
- `GRAPH_RAG_TIMEOUT` controls the bot-to-graph-service request timeout.
...
`--embed-only` reads `data/chunks/all_chunks.json`. Host-side ingestion connects to Chroma at `127.0.0.1:18000`; the Dockerized search API connects to `chroma:8000`.
...
- Host programs use `127.0.0.1:18000`.
